...
//     this.template = fs.readFileSync(emailTemplate, 'utf8');
//   }
...
//     let html = this.replaceTemplateConstant(
//       this.template,
//       '#APP_NAME#',
//       appName,
//     );
...
export class EmailService {
  ...
  constructor() {
    // Define the HTML template directly instead of reading from a file
  ...
  private async sendMail(to: string, subject: string, message: string) {
    ...
    html = this.replaceTemplateConstant(html, '#MESSAGE#', message);
    html = this.replaceTemplateConstant(html, '#SUPPORT_MAIL#', supportMail);
    html = this.replaceTemplateConstant(html, '#APPURL#', process.env.APPURL || '#');
